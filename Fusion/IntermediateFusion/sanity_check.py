...
# Reuse IntermediateFusion model
from train_unet import IntermediateCrossAttentionUNet, count_params
...
def main():
    ...
    with torch.no_grad():
        ...
        fused_bottleneck = model.cross_attn(wet_bottleneck, dry_bottleneck)
        print(f"Cross attention fused bottleneck shape: {tuple(fused_bottleneck.shape)}")
        ...
        gamma_val = float(model.cross_attn.gamma.detach().cpu().item())
        print(f"Current gamma value: {gamma_val:.6f}")

        print("\nSanity check passed.")
        print("Data loading, dual encoders, bottleneck cross attention, and shared decoder are working.")
