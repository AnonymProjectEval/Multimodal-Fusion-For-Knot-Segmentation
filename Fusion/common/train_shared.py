...
def _step_scheduler(scheduler: Any, val_loss_epoch: Optional[float] = None) -> None:
    """
    Safe scheduler step.

    ReduceLROnPlateau needs a monitored value.
    StepLR and most others should be stepped without a value.
    """
    if scheduler is None:
        return

    if isinstance(scheduler, ReduceLROnPlateau):
        if val_loss_epoch is not None and not np.isnan(val_loss_epoch):
    ...
    *,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if should_validate:
            ...
            _step_scheduler(scheduler, val_loss_epoch=val_loss_epoch)
        else:
            _step_scheduler(scheduler, val_loss_epoch=None)
