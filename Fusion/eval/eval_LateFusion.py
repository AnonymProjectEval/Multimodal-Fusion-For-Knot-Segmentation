...
# Update this to your LateFusion best checkpoint
MODEL_PATH = (
    "/net/fs-2/scale/OrionStore/Projects/WaiKnotCT/FullData/"
    "Multimodal/Fusion/LateFusion/Model/checkpoints/best_model.pt"
)

CSV_PATH = str(Path(MODEL_PATH).resolve().parent / "test_metrics_dice_hd95_blob800_latefusion.csv")
...
from LateFusion.train_unet import LateFusionUNet
...
def remove_small_blobs_3d(binary_mask: torch.Tensor, min_size: int = 800, connectivity: int = 2) -> torch.Tensor:
    """
    binary_mask shape can be (D, H, W) or (B, D, H, W).
