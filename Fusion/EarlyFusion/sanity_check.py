...
from train_unet import build_nnunet_direct, count_params
...
def small_crop_3d(x: torch.Tensor, crop_size=(64, 64, 64)) -> torch.Tensor:
    """
    Crop a small center patch for a quick forward pass.
    This avoids using a full 256 cube in the sanity test.
    """
    _, _, d, h, w = x.shape
    cd, ch, cw = crop_size
...
def check_batch(batch: dict, split_name: str) -> None:
    ...
    if vol.shape[1] != 2:
        raise ValueError(f"{split_name} vol must have 2 channels for early fusion. Got {vol.shape[1]}")
    if seg.shape[1] != 1:
        print(f"Warning: {split_name} seg channel is {seg.shape[1]}, expected 1")

    # Channel wise stats for wet and dry
    wet = vol[:, 0:1]
    dry = vol[:, 1:2]
...
def main():
    ...
    # Use cache False for quick startup sanity check
    ...
    # Pull one train batch and one val batch
    ...
    # Build model and run a small forward pass
    ...
    print("\n" + "=" * 80)
    print(
        f"Model params total: {total:,} which is {total / 1e6:.2f}M"
    )
    print(
        f"Model params trainable: {trainable:,} which is {trainable / 1e6:.2f}M"
    )
    print("=" * 80)
    ...
    with torch.no_grad():
        ...
        # Small center crop for a fast forward sanity test
        x_small = small_crop_3d(x, crop_size=(64, 64, 64))
        print(f"\nForward sanity input shape: {tuple(x_small.shape)}")

        y_hat = model(x_small)
        print(f"Forward sanity output shape: {tuple(y_hat.shape)}")
        ...
        print("\nSanity check passed.")
        print("Data loading, early fusion tensor shape, and model forward pass are working.")
